...
export default function AICards() {
  ...
  <div
    ...
    onMouseEnter={() => setHovered(index)}
    onMouseLeave={() => setHovered(null)}
    className={`p-[2px] rounded-[10px] bg-gradient-to-r from-[#272700] to-[#1b1b1b] transition-all duration-500 ${
      hovered === index ? "shadow-[0_0_25px_4px_rgba(255,255,0,0.3)]" : ""
    }`}
  >
    <div className="bg-[#0f0f0f] rounded-[8px] p-6 text-white h-full">
      <img
        src={hovered === index ? card.hoverImage : card.defaultImage}
        alt={card.title}
        className="mb-4 transition-all duration-300"
